...
import { HomePageLoading } from '@/components';
...
const Page = ({ params: { id } }: { params: { id: string } }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            setLoading(true);
            const res = await axiosInstance.put(`/api/v1/books/${id}`, formData, { headers: "multipart/form-data" });
        ...
        if (session.status == 'authenticated' && id) {
            axiosInstance.get(`/api/v1/books/getBookById/${id}`)
                .then((res) => {
                    console.log(res.data);
                    if (res.data.succes) {
                        const data = res.data.payload as BookInfoProps;
                        setGenres(data.genre);
                        setBookInfo({ author: data.author, title: data.title, description: data.description });
    ...
    return (
        <form onSubmit={handleSubmit} className='md:px-20 px-5'>
            ...
            {imageUrl && <img alt='book image' className='h-[200px] my-2 mx-auto w-[100px' src={imageUrl} />}
